...
def extract_token_counts_and_model(log_content):
    ...
    for line in log_content.splitlines():
        ...
        claude_match = re.search(r"model='(claude-[^']+)'.*?usage=Usage\(cache_creation_input_tokens=(\d+), cache_read_input_tokens=(\d+), input_tokens=(\d+), output_tokens=(\d+)\)", line)
        if claude_match:
            model_name = claude_match.group(1)
            cache_creation_input_tokens = int(claude_match.group(2))
            cache_read_input_tokens = int(claude_match.group(3))
            input_tokens = int(claude_match.group(4))
            output_tokens = int(claude_match.group(5))
            
            # Calculate total prompt tokens for Claude models
            prompt_tokens = cache_creation_input_tokens + cache_read_input_tokens + input_tokens
            completion_tokens = output_tokens
            
            # Claude models don't have reasoning tokens
            reasoning_tokens = 0
...
def main():
    ...
    parser.add_argument("--results_dir", default="results/sc_universal_gpt-4o-mini-2024-07-18",
